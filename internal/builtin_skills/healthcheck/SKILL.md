---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for GoClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, GoClaw cron scheduling for periodic checks, or version status checks on a machine running GoClaw (laptop, workstation, Pi, VPS).
---

# GoClaw Host Hardening

## Overview

Assess and harden the host running GoClaw, then align it to a user-defined risk tolerance without breaking access. Use GoClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim GoClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. GoClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if GoClaw can access sensitive data.
...
### 2) Run GoClaw security audits (read-only)

As part of the default read-only checks, run `goclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `goclaw security audit` (faster, non-probing)
2. `goclaw security audit --json` (structured output)

Offer to apply GoClaw safe defaults (numbered):

1. `goclaw security audit --fix`

Be explicit that `--fix` only tightens GoClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check GoClaw version/update status (read-only)

As part of the default read-only checks, run `goclaw update status`.
...
- Credential hygiene notes (location of GoClaw creds, prefer disk encryption)
...
After GoClaw install or first hardening pass, run at least one baseline audit and version check:

- `goclaw security audit`
- `goclaw security audit --deep`
- `goclaw update status`

Ongoing monitoring is recommended. Use the GoClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `goclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `goclaw update status`
...
Before creating, `goclaw cron list` and match on exact `name`. If found, `goclaw cron edit <id> ...`.
If not found, `goclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `goclaw update status` (preferred for source checkouts and channels)

## GoClaw command accuracy

Use only supported commands and flags:

- `goclaw security audit [--deep] [--fix] [--json]`
- `goclaw status` / `goclaw status --deep`
- `goclaw health --json`
- `goclaw update status`
- `goclaw cron add|list|runs|run`

Do not invent CLI flags or imply GoClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by GoClaw compaction:
